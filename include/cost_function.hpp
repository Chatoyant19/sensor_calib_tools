...
class vpnp_calib_orin {
 public:
  ...
  static ceres::CostFunction *Create(VPnPData p, Eigen::Matrix3d _inner,
...
class vpnp_calib_pin_auto {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  vpnp_calib_pin_auto(VPnPData p, Eigen::Matrix3d _R_dr_C,
                      Eigen::Vector3d _t_dr_C, Eigen::Matrix3d _inner,
                      Eigen::Vector4d _distor)
      : pd(p),
        R_dr_C(_R_dr_C),
        t_dr_C(_t_dr_C),
  ...
  bool operator()(const T *_q, const T *_t, T *residuals) const {
    ...
    Eigen::Quaternion<T> q_incre{_q[3], _q[0], _q[1], _q[2]};
    Eigen::Matrix<T, 3, 1> t_incre{_t[0], _t[1], _t[2]};
    // Eigen::Vector3d trans_lidar = Eigen::Vector3d(1.38927, 0.000063, 1.22671);
    // Eigen::Matrix<T, 3, 1> t_incre = trans_lidar.cast<T>();
    Eigen::Matrix<T, 3, 1> p_l(T(pd.x), T(pd.y), T(pd.z));
    Eigen::Matrix<T, 3, 3> R_dr_C_T = R_dr_C.cast<T>();
    Eigen::Matrix<T, 3, 1> t_dr_C_T = t_dr_C.cast<T>();
    Eigen::Matrix<T, 3, 1> p_c =
        R_dr_C_T.transpose() * q_incre.toRotationMatrix() * p_l +
        (R_dr_C_T.transpose() * t_incre - R_dr_C_T.transpose() * t_dr_C_T);
    Eigen::Matrix<T, 3, 1> p_2 = innerT * p_c;
    T uo = p_2[0] / p_2[2];
    T vo = p_2[1] / p_2[2];
    const T &fx = innerT.coeffRef(0, 0);
    ...
    // residuals[0] = (ud - T(pd.u)) * T(pd.direction(1)) - (vd - T(pd.v)) *
    // T(pd.direction(0));
    return true;
  }

  static ceres::CostFunction *Create(VPnPData p, Eigen::Matrix3d _R_dr_C,
...
struct TranslationError {
  TranslationError(const Eigen::Vector3d &prior_, double w_)
      : prior(prior_), w(w_) {}

  template<typename T>
  bool operator()(const T *const translation, T *residuals) const {
    Eigen::Map<const Eigen::Matrix<T, 3, 1>> trans(translation);
    Eigen::Map<Eigen::Matrix<T, 3, 1>> res(residuals);
    res = T(w) * (prior.cast<T>() - trans);
    return true;
  }
...
class vpnp_calib_fisheye_ana : public ceres::SizedCostFunction<2, 6> {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  vpnp_calib_fisheye_ana(VPnPData p, Eigen::Matrix3d _R_dr_C,
                         Eigen::Vector3d _t_dr_C, Eigen::Matrix3d _inner,
                         Eigen::Vector4d _distor)
      : pd(p),
        R_dr_C(_R_dr_C),
  ...
  bool Evaluate(double const *const *parameters, double *residuals,
    ...
    Eigen::Map<const Eigen::Matrix<double, 6, 1>> pose_vec(
        parameters[0]);  // Tx_dr_L
    Eigen::Map<Eigen::Matrix<double, 2, 1>> residual(residuals);

    Eigen::Matrix3d R_dr_L =
        exp_R(Eigen::Vector3d(pose_vec[0], pose_vec[1], pose_vec[2]));
    Eigen::Vector3d t_dr_L =
        Eigen::Vector3d(Eigen::Vector3d(pose_vec[3], pose_vec[4], pose_vec[5]));
    ...
    double theta = atan(r);
    double theta_d =
        theta * (1.0 + distor[0] * pow(theta, 2) + distor[1] * pow(theta, 4) +
    ...
    if (jacobians != nullptr && jacobians[0] != nullptr) {
      ...
      Eigen::Map<Eigen::Matrix<double, 2, 6, Eigen::RowMajor>> jacobian(
          jacobians[0]);
      ...
      double dev_theta_d_pc_X = dev_theta_pc_X +
                                3.0 * distor[0] * theta_2 * dev_theta_pc_X +
      ...
      double dev_dx_pc_X =
          tmp_1 * dev_theta_d_pc_X - tmp_2 * dev_r_pc_X + tmp_3 * dev_a_pc_X;

      double dev_theta_pc_Y = (p_c(1) * p_c(2)) / (X2_Y2_Z2 * sqrt(X2_Y2));
      double dev_theta_d_pc_Y = dev_theta_pc_Y +
                                3.0 * distor[0] * theta_2 * dev_theta_pc_Y +
      ...
      double dev_theta_d_pc_Z = dev_theta_pc_Z +
                                3.0 * distor[0] * theta_2 * dev_theta_pc_Z +
      ...
      double dev_dx_pc_Z =
          tmp_1 * dev_theta_d_pc_Z - tmp_2 * dev_r_pc_Z + tmp_3 * dev_a_pc_Z;

      double dev_dy_pc_X = tmp_4 * dev_theta_d_pc_X - tmp_5 * dev_r_pc_X;

      double dev_b_pc_Y = 1.0 / p_c(2);
      double dev_dy_pc_Y =
          tmp_4 * dev_theta_d_pc_Y - tmp_5 * dev_r_pc_Y + tmp_3 * dev_b_pc_Y;

      double dev_b_pc_Z = -(p_c(1) / (p_c(2) * p_c(2)));
      double dev_dy_pc_Z =
          tmp_4 * dev_theta_d_pc_Z - tmp_5 * dev_r_pc_Z + tmp_3 * dev_b_pc_Z;

      Eigen::Matrix3d p_l_cross;
      p_l_cross << 0, -p_l(2), p_l(1), p_l(2), 0, -p_l(0), -p_l(1), p_l(0), 0;
