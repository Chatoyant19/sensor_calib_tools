...
Calibration::Calibration(const std::string& CalibCfgFile, const std::string& ResultPath, 
  ...
  std::vector<std::vector<pcl::PointCloud<pcl::PointXYZI>>> ground_lines_vec;
  // add floor plane constraint
  for(int scene_index = 0; scene_index < scene_num_; ++scene_index) {
    std::string use_pcd_path = pcd_path_ + "/" + std::to_string(scene_index) + "_visual.pcd";
  ...
  if(use_ground_lines_) {
    for(int scene_index = 0; scene_index < scene_num_; ++scene_index) {
      for(auto c:ground_lines_vec[scene_index]) {
        *lidar.plane_line_cloud_vec_[scene_index] += c;
      }
    }
  }
