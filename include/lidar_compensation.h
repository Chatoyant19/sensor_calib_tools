...
using VectorNd = Eigen::Matrix<double, _row, 1>;
template <size_t _dem>
using MatrixNd = Eigen::Matrix<double, _dem, _dem>;
...
class LidarCompensation {
 public:
  ...
  void compensate(const Eigen::Matrix4d& delta_pose, const double& delta_time,
 ...
 private:
  ...
  void deltaPoseToVelocity(const Eigen::Matrix4d& delta_pose,
                           const double& delta_time, Eigen::Vector6d* velocity);
