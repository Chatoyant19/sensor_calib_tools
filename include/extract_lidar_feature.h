...
class ExtractLidarFeature {
 public:
  ...
  ExtractLidarFeature(const double& voxel_size, const double& eigen_ratio,
                      const double& p2line_dis_thre, const double& theta_min,
                      const double& theta_max);
  ExtractLidarFeature(const double& voxel_size,
                      const double& ransac_dis_threshold,
                      const int& plane_size_threshold,
                      const double& p2line_dis_thre, const double& theta_min,
                      const double& theta_max);

  void getEdgeFeaturesByAdaVoxel(
      const pcl::PointCloud<pcl::PointXYZI>::Ptr& input_lidar_cloud,
      pcl::PointCloud<pcl::PointXYZI>::Ptr& lidar_edge_cloud);
  void getEdgeFeatures(
      const pcl::PointCloud<pcl::PointXYZI>::Ptr& input_lidar_cloud,
      pcl::PointCloud<pcl::PointXYZI>::Ptr& lidar_edge_cloud);
 ...
 private:
  ...
  void estimateEdgeByAdaVoxel(
      const std::unordered_map<VOXEL_LOC, OCTO_TREE_ROOT*>& surf_map,
      pcl::PointCloud<pcl::PointXYZI>::Ptr& lidar_edge_cloud);
  void estimatePlaneByAdaVoxel(OCTO_TREE_ROOT* surf,
                               std::vector<Plane*>& merge_plane_list);
  void mergePlane(const std::vector<Plane*>& origin_list,
  ...
  Eigen::Vector3d computeLineDirection(
