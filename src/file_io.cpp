...
namespace file_io {
bool readProtoFromTextFile(const std::string& file,
...
bool readCamInFromXmlFile(const std::string& xml_file, std::string& model_type,
  ...
  std::ifstream file_if(xml_file);
  if(!file_if) {
    std::cerr << "Failed to open camera intrinsic file: " << xml_file << std::endl;
    return false;
  }
  ...
  for (rapidxml::xml_node<>* param = doc.first_node("param")->first_node();
...
bool readCamExFromYmlFile(const std::string& yml_file,
  ...
  if (!fCamExSet.isOpened()) {
    std::cerr << "Failed to open cams extrinsic file at " << yml_file
              << std::endl;
    return false;
  }
  ...
  cam_extrinsic << static_cast<double>(fixed_ext.at<float>(0, 0)),
      static_cast<double>(fixed_ext.at<float>(0, 1)),
      static_cast<double>(fixed_ext.at<float>(0, 2)),
      static_cast<double>(fixed_ext.at<float>(0, 3)),
      static_cast<double>(fixed_ext.at<float>(1, 0)),
      static_cast<double>(fixed_ext.at<float>(1, 1)),
      static_cast<double>(fixed_ext.at<float>(1, 2)),
      static_cast<double>(fixed_ext.at<float>(1, 3)),
      static_cast<double>(fixed_ext.at<float>(2, 0)),
      static_cast<double>(fixed_ext.at<float>(2, 1)),
      static_cast<double>(fixed_ext.at<float>(2, 2)),
      static_cast<double>(fixed_ext.at<float>(2, 3)), 0.0, 0.0, 0.0, 1.0;
  return true;
}
...
bool readExtrinsicFromYamlFile(const std::string& file,
  ...
  if (!settings.isOpened()) {
    std::cerr << "# ERROR: Failed to open settings file at: " << file
              << std::endl;
    return false;
  }
...
bool writeProtoToTextFile(std::string& file,
  ...
  int fd = open(file.c_str(), O_WRONLY | O_CREAT | O_TRUNC,
                0644);  // 0644 is the file permission
  if (fd == -1) {
...
void loadPcdFilePath(const std::string& pcd_folder,
  ...
  if (glob(pcds_path.c_str(), GLOB_ERR, NULL, &pglob)) {
    std::cerr << "Glob lib can not match any path for pattern : " << pcds_path
              << std::endl;
    return;
  }
...
bool read_timestamp_info(const std::string& input_img_stamp_file,
  ...
  while (!infile.eof()) {
    ...
    if (flag) img_stamps.push_back(double(time * 1.0e-6));
  }
  infile.close();
...
int getSyncTimestampIdex(const std::vector<double>& timestamp_vec,
  ...
  auto it = std::lower_bound(timestamp_vec.begin(), timestamp_vec.end(),
                             pcd_start_time);
  if (it == timestamp_vec.begin()) {
    ...
    img_start_time =
        (std::abs(pcd_start_time - *it) > std::abs(pcd_start_time - *prev))
            ? *prev
            : *it;
  } else {
  ...
  if (std::abs(pcd_start_time - img_start_time) > 0.02) {
    ...
    for (int pose_index = 0; pose_index < stamp_pose_vec->size();
         ++pose_index) {
      dr_stamp_vec.emplace_back(stamp_pose_vec->at(pose_index).first);
    }

    if (pcd_start_time <= img_start_time) {
      auto dr_it = std::lower_bound(dr_stamp_vec.begin(), dr_stamp_vec.end(),
                                    pcd_start_time);
      if (dr_it == dr_stamp_vec.begin())
        dr_start_still_time = *dr_it;
      ...
      dr_it = std::lower_bound(dr_stamp_vec.begin(), dr_stamp_vec.end(),
                               img_start_time);
      if (dr_it != dr_stamp_vec.end())
        dr_end_still_time = *dr_it;
      else {
        std::cerr << "dr stamp_pose is bad." << std::endl
    ...
    while (dr_cnt < stamp_pose_vec->size()) {
      if (stamp_pose_vec->at(dr_cnt).first < dr_start_still_time) {
        ...
        curr_q = Eigen::Quaterniond(
            stamp_pose_vec->at(dr_cnt).second.block<3, 3>(0, 0));
      ...
      curr_q = Eigen::Quaterniond(
          stamp_pose_vec->at(dr_cnt).second.block<3, 3>(0, 0));
      curr_t = stamp_pose_vec->at(dr_cnt).second.block<3, 1>(0, 3);
      if ((curr_q.w() == last_q.w()) && (curr_q.x() == last_q.x()) &&
          (curr_q.y() == last_q.y()) && (curr_q.z() == last_q.z()) &&
          (curr_t.x() == last_t.x()) && (curr_t.y() == last_t.y()) &&
          (curr_t.z() == last_t.z())) {
        ++dr_cnt;
        last_q = curr_q;
        last_t = curr_t;
        ...
        std::cerr
            ...
            << std::endl
            << "vehicle starts moving, " << stamp_pose_vec->at(dr_cnt).first
            << std::endl;
        exit(-1);
      }

      if (stamp_pose_vec->at(dr_cnt).first == dr_end_still_time) break;
    }
  }
