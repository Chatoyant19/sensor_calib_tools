...
#define PLV(a)                             \
  std::vector<Eigen::Matrix<double, a, 1>, \
...
struct hash<VOXEL_LOC> {
  size_t operator()(const VOXEL_LOC &s) const {
    ...
    // hash<int64_t>()(s.y))*HASH_P)%MAX_N + hash<int64_t>()(s.x);
...
class VOX_FACTOR {
 public:
  ...
  void push(const Eigen::Vector3d &vec) {
    ...
    Bi << 2 * vec(0), 0, 0, vec(1), vec(0), 0, vec(2), 0, vec(0), 0, 2 * vec(0),
    ...
    Eigen::Matrix3d p_cov =
        (dis_error - co_ang) * w * w.transpose() + co_ang * I33;
...
class OCTO_TREE_NODE {
 public:
  ...
  OCTO_TREE_NODE(double _eigen_thr = 1.0 / 10, int _layer_limit = 3,
                 double _what = 0.98)
  ...
  bool judge_eigen(int layer) {
    ...
    eigen_ratio =
        saes.eigenvalues()[0] / saes.eigenvalues()[2];  // [0] is the smallest

    if (eigen_ratio > eigen_thr) return 0;
    if (saes.eigenvalues()[0] / saes.eigenvalues()[1] > 0.1)
      return 0;  // 排除线状点云
    ...
    for (int i = 0; i < 8; i++)
      if (covMats[i].N > MIN_PT) {
        ...
        if (fabs(child_direct.dot(direct)) > what) num_qua++;
        num_all++;
      }
  ...
  void cut_func() {
    ...
    for (uint j = 0; j < a_size; j++) {
      int xyz[3] = {0, 0, 0};
      for (uint k = 0; k < 3; k++)
        if (pvec_orig[j][k] > voxel_center[k]) xyz[k] = 1;
      int leafnum = 4 * xyz[0] + 2 * xyz[1] + xyz[2];
  ...
  void recut() {
    if (octo_state == UNKNOWN) {
      ...
      if (judge_eigen(layer)) {
        ...
        for (auto pt : vec_orig) plane_ptr->plane_points.push_back(pt);
  ...
  void get_plane_list(std::vector<Plane *> &plane_list) {
    if (octo_state == PLANE)
      ...
      for (int i = 0; i < 8; i++)
        if (leaves[i] != nullptr) leaves[i]->get_plane_list(plane_list);
  }
};
...
static void downsample_voxel(const pcl::PointCloud<pcl::PointXYZI> &pc,
                             pcl::PointCloud<pcl::PointXYZI> &pc_sam,
                             double voxel_size) {
