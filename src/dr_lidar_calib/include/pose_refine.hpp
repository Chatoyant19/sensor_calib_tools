...
class LM_OPTIMIZER {
 public:
  ...
  void get_center(vector_vec3d& origin_pc, int cur_frame,
                  vector_vec3d& origin_point, std::vector<int>& window_num,
                  int filternum2use) {
  ...
  Eigen::Quaterniond exp_q(const Eigen::Vector3d& omega) {
  ...
  void optimize() {
    ...
    for (int loop = 0; loop < 20; loop++) {
      ...
      for (int i = 0; i < window_size; i++) {
        Eigen::Quaterniond q_tmp(exp_q(dxi.block<3, 1>(6 * i, 0)) * poses[i]);
      ...
      if (std::isnan(residual1) || std::isnan(residual2)) {
        std::cerr << "residual is nan" << std::endl;
        exit(-1);
      }
  ...
  void evaluate_only_residual(const vector_quad& poses_,
                              const vector_vec3d& ts_, double& residual) {
    residual = 0;
    size_t voxel_size = origin_points.size();
...
class OCTO_TREE {
 public:
  ...
  OCTO_TREE(int window_size, double eigen_limit)
      : win_size(window_size), eigen_ratio(eigen_limit) {
    octo_state = UNKNOWN;
    layer = 0;
  ...
  void recut() {
    if (octo_state == UNKNOWN) {
      points_size = 0;
      for (int i = 0; i < win_size; i++) points_size += origin_pc[i]->size();
      ...
      if (judge_eigen()) {
        ...
        for (int i = 0; i < win_size; i++) {
          ...
          for (uint j = 0; j < pt_size; j++) {
            int xyz[3] = {0, 0, 0};
            for (uint k = 0; k < 3; k++)
              if ((*transform_pc[i])[j][k] > voxel_center[k]) xyz[k] = 1;
  ...
  void tras_display(pcl::PointCloud<pcl::PointXYZINormal>::Ptr& cloud) {
    ...
    if (octo_state == PLANE) {
      ...
      layer++;
      for (int i = 0; i < 8; i++)
        if (leaves[i] != nullptr) leaves[i]->tras_display(cloud);
    }
  }
...
static void cut_voxel(std::unordered_map<VOXEL_LOC, OCTO_TREE*>& feature_map,
                      const pcl::PointCloud<pcl::PointXYZI>::Ptr& feature_pts,
                      const Eigen::Matrix4d& pose, int f_head, int window_size,
                      double voxel_size, double eigen_threshold) {
  uint pt_size = feature_pts->size();
  for (uint i = 0; i < pt_size; i++) {
    pcl::PointXYZI& pt = feature_pts->points[i];
    Eigen::Vector4d pt_tmp(pt.x, pt.y, pt.z, 1.0);
    Eigen::Vector3d pt_origin(pt.x, pt.y, pt.z);
    Eigen::Vector3d pt_trans = (pose * pt_tmp).head<3>();
    ;
