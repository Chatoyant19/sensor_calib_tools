...
class CameraOdometerError2 {
 public:
  ...
  bool operator()(const T* const q4x1, const T* const t3x1,
    ...
    Eigen::Matrix<T, 3, 1> t_err =
        (R1 - Eigen::Matrix<T, 3, 3>::Identity()) * t - (R * t2) + t1;
...
class HandEyeCalib {
 ...
 private:
  ...
  void resample_quaternions(const StampedPoseVectorPtr& poses, const double& dt,
  ...
  std::vector<double> linear_interp(const std::vector<double>& x,
                                    std::vector<double>& xp,
                                    const std::vector<double>& fp);
  void compute_angular_velocity_norms(
  ...
  Eigen::Vector3d angular_velocity_between_quaternions(
      const Eigen::Quaterniond& q0, const Eigen::Quaterniond& q1,
      const double& dt);
  double calculate_time_offset_from_signals(
  ...
  std::vector<double> calculateCorrelation(const std::vector<double>& vec1,
                                           const std::vector<double>& vec2);
  vector_vec3d filter_and_smooth_angular_velocity(
