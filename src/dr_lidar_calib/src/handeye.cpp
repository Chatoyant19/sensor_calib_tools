...
void HandEyeCalib::processingPoses() {
  ...
  std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d> > T_seq1, T_seq2;
  ...
  std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d> > relative_seq1, relative_seq2;
...
double HandEyeCalib::calc_time_offset() {
  ...
  compute_angular_velocity_norms(quatenoins_interp_seq1, sample_seq1, smoothing_kernel_size1_, 
                                 clipping_percentile1_, angular_velocity_norms1, angular_vel_seq1);
  compute_angular_velocity_norms(quatenoins_interp_seq2, sample_seq2, smoothing_kernel_size2_, 
                                 clipping_percentile2_, angular_velocity_norms2, angular_vel_seq2);
  ...
  time_offset = calculate_time_offset_from_signals(sample_seq1, angular_velocity_norms1,
                                     sample_seq2, angular_velocity_norms2); 
  return time_offset;
}

double HandEyeCalib::calcTimeDiffMean(const std::vector<double>& timestamp_seq) {
  double time_diff = 0.;
...
void HandEyeCalib::resample_quaternions(const StampedPoseVectorPtr& poses,
  ...
  Eigen::VectorXd new_sample_seq = Eigen::VectorXd::LinSpaced(new_time_seq_size, t_start, t_end);
  for(int i = 0; i< new_time_seq_size; ++i) {
    sample_seq.emplace_back(new_sample_seq[i]);
  }

  StampedPoseVectorPtr new_timestamp_pose_seq = std::make_shared<StampedPoseVector>();
  ...
  for(int i = 0; i < new_timestamp_pose_seq->size(); ++i) {
    Eigen::Quaterniond q = Eigen::Quaterniond(new_timestamp_pose_seq->at(i).second.block<3, 3>(0, 0));
...
void HandEyeCalib::interpolate_poses_from_samples(
  ...
  //get new tx,ty,tz
  std::vector<double> new_tx_seq = linear_interp(samples, timestamp_seq, tx_seq);
  std::vector<double> new_ty_seq = linear_interp(samples, timestamp_seq, ty_seq);
  std::vector<double> new_tz_seq = linear_interp(samples, timestamp_seq, tz_seq);
  ...
  for (int i = 0; i < samples.size(); i++) {
    ...
    Eigen::Vector3d p = Eigen::Vector3d(new_tx_seq[i], new_ty_seq[i], new_tz_seq[i]);
    if(tmp1 <= tmp2) {
      Eigen::Matrix4d T = Eigen::Matrix4d::Identity();
      T.block<3, 3>(0, 0) = timestamp_pose_seq->at(right_idx).second.block<3, 3>(0, 0);
      T.block<3, 1>(0, 3) = p;
      ...
      assert(left_idx >= 0);
      double times_scaled = (samples[i] - timestamp_seq[left_idx]) / (timestamp_seq[right_idx] - timestamp_seq[left_idx]);
      assert(times_scaled >= 0.);
      assert(times_scaled <= 1.0);
      double dot_product = 0.;
      double theta_prime = 0., theta = 0.;
      Eigen::Quaterniond q_1 = Eigen::Quaterniond(timestamp_pose_seq->at(left_idx).second.block<3, 3>(0, 0));
      Eigen::Quaterniond q_2 = Eigen::Quaterniond(timestamp_pose_seq->at(right_idx).second.block<3, 3>(0, 0));
      Eigen::Quaterniond q;
      ...
      if(times_scaled == 0.)
        ...
        if(dot_product < 0.) {
          ...
          // q_1 = q_1*(-1);
          q_1 = Eigen::Quaterniond(q_1.w() * (-1), q_1.x() * (-1), q_1.y() * (-1), q_1.z() * (-1));
          // wd to check
        ...
        Eigen::Quaterniond q_2_tmp = Eigen::Quaterniond(q_2.w() * dot_product, q_2.x() * dot_product, q_2.y() * dot_product, q_2.z() * dot_product);
        Eigen::Quaterniond q_3 = Eigen::Quaterniond(q_1.w() - q_2_tmp.w(), q_1.x() - q_2_tmp.x(), q_1.y() - q_2_tmp.y(), q_1.z() - q_2_tmp.z());
        
        q_3.normalize();
        // wd to check 
        // q = q_1*cos(theta) + q_3 * sin(theta);
        Eigen::Quaterniond q_1_tmp = Eigen::Quaterniond(q_1.w() * cos(theta), q_1.x() * cos(theta), q_1.y() * cos(theta), q_1.z() * cos(theta));
        Eigen::Quaterniond q_3_tmp = Eigen::Quaterniond(q_3.w() * sin(theta), q_3.x() * sin(theta), q_3.y() * sin(theta), q_3.z() * sin(theta));
        q = Eigen::Quaterniond(q_1_tmp.w() + q_3_tmp.w(), q_1_tmp.x() + q_3_tmp.x(), q_1_tmp.y() + q_3_tmp.y(), q_1_tmp.z() + q_3_tmp.z());
      }
      Eigen::Matrix4d T = Eigen::Matrix4d::Identity();
...
std::vector<double> HandEyeCalib::linear_interp(const std::vector<double>& x, std::vector<double>& xp, const std::vector<double>& fp) {
...
void HandEyeCalib::compute_angular_velocity_norms(const vector_quad& quatenoins_interp_seq,
  ...
  for(int i = 1; i < angular_velocity_size; ++i) {
    ...
    Eigen::Vector3d angular_vel = 
        angular_velocity_between_quaternions(quatenoins_interp_seq[i-1], quatenoins_interp_seq[i], dt);                          
    angular_vel_seq.emplace_back(angular_vel);
  } 
  vector_vec3d angular_velocity_filtered_seq = filter_and_smooth_angular_velocity(angular_vel_seq, smoothing_kernel_size, clipping_percentile);
  for(int i = 0; i < angular_velocity_size-1; ++i) {
    angular_velocity_norms.emplace_back(angular_velocity_filtered_seq[i].norm()); 
  }
  assert(angular_velocity_norms.size() == quatenoins_interp_seq.size()-1);
}

Eigen::Vector3d HandEyeCalib::angular_velocity_between_quaternions(const Eigen::Quaterniond& q0, 
                                                     const Eigen::Quaterniond& q1,
                                                     const double& dt) {
  Eigen::Quaterniond q0_q1_inv = q0 * q1.inverse();
  // std::cout << q0_q1_inv.x() << " " << q0_q1_inv.y() << " " << q0_q1_inv.z() << " " << q0_q1_inv.w() << std::endl;
  if(q0_q1_inv.w() < 0.) {
    // q0_q1_inv = q0_q1_inv * (-1.0);
    q0_q1_inv = Eigen::Quaterniond(q0_q1_inv.w() * (-1.0), q0_q1_inv.x() * (-1.0), q0_q1_inv.y() * (-1.0), q0_q1_inv.z() * (-1.0));
  }
  Eigen::AngleAxisd angle_axis(q0_q1_inv);
  return 0.5 / dt * angle_axis.angle() * angle_axis.axis();
} 

vector_vec3d HandEyeCalib::filter_and_smooth_angular_velocity(const vector_vec3d& angular_vel_seq,
  ...
  // std::cout << "Clipping angular velocity norms to : " << max_value << "rad/s ..." << std::endl;
  vector_vec3d angular_velocity_clipped(angular_vel_seq.size(), Eigen::Vector3d(0.,0.,0.));
  ...
  for(int i = 0; i < angular_velocity_clipped.size(); ++i) {
    if(angular_velocity_clipped[i].x() < (max_value*(-1.0)) ) angular_velocity_clipped[i].x() = max_value*(-1.0);
    if(angular_velocity_clipped[i].x() > max_value ) angular_velocity_clipped[i].x() = max_value;
    if(angular_velocity_clipped[i].y() < (max_value*(-1.0)) ) angular_velocity_clipped[i].y() = max_value*(-1.0);
    if(angular_velocity_clipped[i].y() > max_value ) angular_velocity_clipped[i].y() = max_value;
    if(angular_velocity_clipped[i].z() < (max_value*(-1.0)) ) angular_velocity_clipped[i].z() = max_value*(-1.0);
    if(angular_velocity_clipped[i].z() > max_value ) angular_velocity_clipped[i].z() = max_value;
  }
  // std::cout << "Done clipping angular velocity norms..." << std::endl;
  double low_pass_kernel_value = 1.0/(double)low_pass_kernel_size;
  std::vector<double> low_pass_kernel(low_pass_kernel_size, low_pass_kernel_value);
  // std::cout << "low_pass_kernel: " << low_pass_kernel.size() << std::endl;
  // std::cout << "Smoothing with kernel size " << low_pass_kernel_size << "samples..." << std::endl;
  vector_vec3d angular_velocity_smoothed= calculateEigenCorrelation(angular_velocity_clipped, low_pass_kernel);        
  // std::cout << "Done smoothing angular velocity norms..." << std::endl;
  return angular_velocity_smoothed;
}

double HandEyeCalib::calculatePercentile(const vector_vec3d& angular_vel_seq, double p) {
...
vector_vec3d HandEyeCalib::calculateEigenCorrelation(const vector_vec3d & vec1, 
...
double HandEyeCalib::calculate_time_offset_from_signals(const std::vector<double>& sample_seq1,
  ...
  std::vector<double> convoluted_signals = calculateCorrelation(angular_velocity_norms2, angular_velocity_norms1);
  ...
  std::vector<double>::iterator iter = std::max_element(convoluted_signals.begin(), convoluted_signals.end());
  int max_index = std::distance(convoluted_signals.begin(), iter);
  int offset_index = offset_indices[max_index];
  double time_offset = sample_diff1 * (double)offset_index + sample_seq2.at(0) - sample_seq1.at(0);
  return time_offset;
}

std::vector<double> HandEyeCalib::calculateCorrelation(const std::vector<double>& vec1, const std::vector<double>& vec2) {
  int M = vec1.size();
  int N = vec2.size();
...
void HandEyeCalib::compute_aligned_poses(const double& time_offset,
  ...
  double start_time = std::max(timestamp_pose_shifted_seq1->begin()->first, poses_2_->begin()->first);
  double end_time = std::min(timestamp_pose_shifted_seq1->end()->first, poses_2_->end()->first);
  ...
  for(int i = 0; i < timestamp_pose_shifted_seq1->size(); ++i) {
    timestamp_shifted_seq1.emplace_back(timestamp_pose_shifted_seq1->at(i).first);
  }
  //get timestamp_seq2
  ...
  for(int i = 0; i < timestamps_low.size(); ++i) {
    ...
    right_it = std::lower_bound(timestamps_high.begin(), timestamps_high.end(), timestamps_low.at(i));
    idx = std::distance(timestamps_high.begin(), right_it);
    if(idx >= timestamps_high.size()-1) {
      // std::cout << "Omitting timestamps at the end of the high frequency poses." << std::endl;
      break;
    }
    ...
    if((timestamps_low.at(i) - timestamps_high.at(left_idx)) < max_time_stamp_difference
       && (timestamps_high.at(right_idx) - timestamps_low.at(i)) <= max_time_stamp_difference) {
      samples.emplace_back(timestamps_low.at(i));
    }
  }

  interpolate_poses_from_samples(timestamp_pose_shifted_seq1, samples, aligned_pose_seq1);
...
bool HandEyeCalib::estimate(Eigen::Matrix4d& H_odo_cam) const {
  ...
  for(size_t i = 0; i < 2; ++i) {
    ...
    for (int segmentId = 0; segmentId < segmentCount; ++segmentId)
    {
      for (size_t motionId = 0; motionId < rvecs1_.at(segmentId).size(); ++motionId)
      {
...
//        Eigen::Vector3d pc = tvec2;
    ...
    H_odo_cam.block<3,3>(0,0) = Eigen::AngleAxisd(alpha_best, Eigen::Vector3d::UnitZ()) * R_yx;
    ...
    for (int segmentId = 0; segmentId < segmentCount; ++segmentId)
    {
        for (size_t motionId = 0; motionId < rvecs1_.at(segmentId).size(); ++motionId)
        {
            ...
            total_err += ((R1 - Eigen::Matrix3d::Identity()) * t
                                     - (rot * t2) + t1).norm();
        }
...
bool HandEyeCalib::estimateRyx(Eigen::Matrix3d& R_yx_1, Eigen::Matrix3d& R_yx_2) const {
...
void HandEyeCalib::refineEstimate(Eigen::Matrix4d& H_odo_cam) const {
    ...
    for (size_t i = 0; i < rvecs1_.size(); ++i)
    {
        for (size_t j = 0; j < rvecs1_.at(i).size(); ++j)
        {
            ...
            //         new CameraOdometerError1(rvecs1.at(i).at(j), tvecs1.at(i).at(j), rvecs2.at(i).at(j), tvecs2.at(i).at(j)));

          
          ceres::CostFunction* costFunction =
              // t is only flexible on x and y.
              new ceres::AutoDiffCostFunction<CameraOdometerError2, 6, 4, 3>(
                  new CameraOdometerError2(rvecs1_.at(i).at(j), tvecs1_.at(i).at(j), rvecs2_.at(i).at(j), tvecs2_.at(i).at(j)));
...
bool HandEyeCalib::solveQuadraticEquation(double a, double b, double c, double& x1, double& x2) const {
    if (fabs(a) < 1e-12)
    {
        x1 = x2 = -c / b;
        return true;
