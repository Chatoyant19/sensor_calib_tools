...
LidarOdometry::LidarOdometry(const bool& use_compensation) {
  ...
  pointcloud_filtered_ = pcl::PointCloud<pcl::PointXYZI>::Ptr(
      new pcl::PointCloud<pcl::PointXYZI>());

  is_odom_inited_ = false;

  // init local map
  // laserCloudCornerMap_ = pcl::PointCloud<pcl::PointXYZI>::Ptr(new
  // pcl::PointCloud<pcl::PointXYZI>());
  laserCloudSurfMap_ = pcl::PointCloud<pcl::PointXYZI>::Ptr(
      new pcl::PointCloud<pcl::PointXYZI>());
  ...
  // downSizeFilterEdge_.setLeafSize(map_resolution_, map_resolution_,
  // map_resolution_);
  downSizeFilterSurf_.setLeafSize(map_resolution_ * 2, map_resolution_ * 2,
                                  map_resolution_ * 2);

  // kd-tree
  kdtreeSurfMap_ = pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr(
      new pcl::KdTreeFLANN<pcl::PointXYZI>());
}

void LidarOdometry::runOdometry(
...
void LidarOdometry::addPointCloud(
    const pcl::PointCloud<pcl::PointXYZI>::Ptr& origin_pcd) {
...
void LidarOdometry::downSamplingToMap(
...
void LidarOdometry::addSurfCostFactor(
  ...
  for (int i = 0; i < (int)pc_in->points.size(); i++) {
    ...
    kdtreeSurfMap_->nearestKSearch(point_temp, 5, pointSearchInd,
                                   pointSearchSqDis);
...
void LidarOdometry::pointAssociateToMap(pcl::PointXYZI const* const pi,
...
void LidarOdometry::addPointsToMap(
