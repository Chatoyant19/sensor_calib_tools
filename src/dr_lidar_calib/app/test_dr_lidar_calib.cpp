...
/*void loadConfigFile(const std::string& calib_setting_path, DrLidarCalibParam& calib_param) {
  ...
  int scene_num = fSettings["SceneNum"];
  ...
  for(auto name: cams_name_vec)
    std::cout << name << ", ";
  std::cout << std::endl;
  std::cout << "SceneNum: " << scene_num << std::endl;
  ...
  for(size_t cam_index = 0; cam_index < cams_name_vec.size(); ++cam_index) {
    ...
    for(size_t scene_index = 0; scene_index < scene_num; ++scene_index) {
      std::string img_path = cam_path + "/" + std::to_string(scene_num) + "." + img_file_extension;
      cv::Mat raw_img = cv::imread(img_path);
      calib_param.images[cam_index].emplace_back(raw_img);
    }
  }

  calib_param.map_pcd_vec_.resize(scene_num);
  calib_param.visual_pcd_vec_.resize(scene_num);
  for(size_t scene_index = 0; scene_index < scene_num; ++scene_index) {
  ...
  for(size_t cam_index = 0; cam_index < cams_name_vec.size(); ++cam_index) {
    ...
    file_io::readCamInFromXmlFile(cam_intrinsics_path, calib_param.cams_model_vec[cam_index], calib_param.camera_matrix_vec[cam_index],
