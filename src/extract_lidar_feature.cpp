...
ExtractLidarFeature::ExtractLidarFeature(const double& voxel_size,
...
void ExtractLidarFeature::getEdgeFeaturesByAdaVoxel(
...
void ExtractLidarFeature::cutVoxel(
...
void ExtractLidarFeature::estimateEdgeByAdaVoxel(
    const std::unordered_map<VOXEL_LOC, OCTO_TREE_ROOT*>& surf_map,
    pcl::PointCloud<pcl::PointXYZI>::Ptr& lidar_edge_cloud) {
...
void ExtractLidarFeature::estimatePlaneByAdaVoxel(
    OCTO_TREE_ROOT* surf, std::vector<Plane*>& merge_plane_list) {
...
void ExtractLidarFeature::mergePlane(const std::vector<Plane*>& origin_list,
  ...
  for (size_t i = 0; i < origin_list.size(); i++) {
    ...
    merge_plane->normal << evecs.real()(0, evalsMin), evecs.real()(1, evalsMin),
...
void ExtractLidarFeature::extractLineByAdaVoxel(
    const std::vector<Plane*>& merge_plane_list,
  ...
  for (size_t p1_index = 0; p1_index < merge_plane_list.size() - 1;
       p1_index++) {
    for (size_t p2_index = p1_index + 1; p2_index < merge_plane_list.size();
         p2_index++) {
...
void ExtractLidarFeature::projectLine(
...
void ExtractLidarFeature::mergeLine(
    const std::vector<pcl::PointCloud<pcl::PointXYZI>>& origin_line_list,
    std::vector<pcl::PointCloud<pcl::PointXYZI>>& merge_line_list) {
  ...
  for (size_t index1 = 0; index1 < origin_line_list.size(); ++index1) {
    ...
    for (size_t index2 = 0; index2 < merge_tmp.size(); ++index2) {
      Eigen::Vector3d line1_direct =
          computeLineDirection(origin_line_list[index1]);
  ...
  for (const auto& line : merge_tmp) {
    if (line.size() > line_points_nums_) merge_line_list.emplace_back(line);
  }
}

Eigen::Vector3d ExtractLidarFeature::computeLineDirection(
    const pcl::PointCloud<pcl::PointXYZI>& line) {
...
double ExtractLidarFeature::cosineSimilarity(const Eigen::Vector3d& v1,
                                             const Eigen::Vector3d& v2) {
  double dotProduct = v1.x() * v2.x() + v1.y() * v2.y() + v1.z() * v2.z();
  double magnitude1 =
      std::sqrt(v1.x() * v1.x() + v1.y() * v1.y() + v1.z() * v1.z());
  double magnitude2 =
      std::sqrt(v2.x() * v2.x() + v2.y() * v2.y() + v2.z() * v2.z());

  return dotProduct / (magnitude1 * magnitude2);
}

ExtractLidarFeature::ExtractLidarFeature(const double& voxel_size,
                                         const double& ransac_dis_threshold,
...
void ExtractLidarFeature::getEdgeFeatures(
    const pcl::PointCloud<pcl::PointXYZI>::Ptr& input_lidar_cloud,
    pcl::PointCloud<pcl::PointXYZI>::Ptr& lidar_edge_cloud) {
...
void ExtractLidarFeature::initVoxel(
...
void ExtractLidarFeature::estimateEdge(
  ...
  for (auto iter = voxel_map.begin(); iter != voxel_map.end(); iter++) {
    ...
    std::vector<pcl::PointCloud<pcl::PointXYZI>> line_cloud_list;
    calcLine(merge_plane_list, iter->second->voxel_origin, line_cloud_list);
    // ouster 5,normal 3
    if (line_cloud_list.size() > 0 && line_cloud_list.size() <= 8) {
      for (const auto& line : line_cloud_list) *lidar_edge_cloud += line;
    }
...
void ExtractLidarFeature::estimatePlane(
    Voxel* voxel, std::vector<SinglePlane>& merge_plane_list) {
  // 创建一个体素滤波器
  ...
  while (cloud_filter->points.size() > 10) {
    ...
    if (inliers->indices.size() == 0) {
      std::cerr << "Could not estimate a planner model for the given dataset"
                << std::endl;
      break;
    }
...
void ExtractLidarFeature::mergePlane_1(std::vector<SinglePlane>& merge_list) {
  ...
  std::sort(merge_list.begin(), merge_list.end(),
            [](const SinglePlane& p1, const SinglePlane& p2) {
              return p1.cloud.size() > p2.cloud.size();
            });

  for (size_t plane_index1 = 0; plane_index1 < merge_list.size();
       ++plane_index1) {
    SinglePlane plane1 = merge_list[plane_index1];
    for (size_t plane_index2 = plane_index1 + 1;
         plane_index2 < merge_list.size(); ++plane_index2) {
      ...
      auto dist_vec =
          (plane1.p_center.getVector3fMap() - plane2.p_center.getVector3fMap())
              .cast<double>();
      float max_dist =
          std::max(dist_vec.dot(plane1.normal), dist_vec.dot(plane2.normal));
      if (max_dist > 0.2) continue;

      plane1.p_center.getVector3fMap() =
          (plane1.p_center.getVector3fMap() * plane1.cloud.size() +
...
void ExtractLidarFeature::calcLine(
    const std::vector<SinglePlane>& plane_list, const Eigen::Vector3d origin,
    std::vector<pcl::PointCloud<pcl::PointXYZI>>& line_cloud_list) {
        ...
        if (theta > theta_max_ && theta < theta_min_) {
          ...
          if (plane_list[plane_index1].cloud.size() > 0 &&
            ...
            if (points.size() == 2) {
              ...
              Eigen::Vector3f step =
                  (p2.getVector3fMap() - p1.getVector3fMap()) / length;
              ...
              for (float inc = 0; inc <= length; inc += step_size) {
                ...
                if ((kdtree1->nearestKSearch(p, K, pointIdxNKNSearch1,
                  ...
                  float dis1 = (p.getVector3fMap() -
                                plane_list[plane_index1]
                                    .cloud.points[pointIdxNKNSearch1[0]]
                                    .getVector3fMap())
                                   .squaredNorm();
                  float dis2 = (p.getVector3fMap() -
                                plane_list[plane_index2]
                                    .cloud.points[pointIdxNKNSearch2[0]]
                                    .getVector3fMap())
                                   .squaredNorm();
                  if (std::max(dis1, dis2) <
                      p2line_dis_thre_ * p2line_dis_thre_) {
                    line_cloud.push_back(p);
                    if (start_inc < 0) start_inc = inc;
                    end_inc = inc;
                  }
                }
              }
              ...
              if (line_cloud.size() > 10 &&
                  line_cloud.size() > 0.5 * (end_inc - start_inc) / step_size) {
                line_cloud_list.emplace_back(line_cloud);
              }
            }
          }
