...
bool MultiLidarsCalib::processLidar(
  ...
  for (auto p : *input_pcd) {
    if ((!std::isfinite(p.x) || !std::isfinite(p.y) || !isfinite(p.z)) ||
        ...
        (std::sqrt(p.x * p.x + p.y * p.y + p.z * p.z) > lidar.max_dis))
      continue;
    pcd->push_back(p);
  }
  Eigen::Matrix4d pose;
  ...
  if (lidar.lidar_type == LidarType::Hesai64 ||
      lidar.lidar_type == LidarType::Bp32 ||
...
bool MultiLidarsCalib::lidarCompensate(
  ...
  lidar.compens->compensate(delta_pose, delta_timestamp, origin_pcd,
                            compensated_pcd);
...
void MultiLidarsCalib::runBaseLidar(const StampedPcdVectorPtr &pcds_seq,
  ...
  for (size_t i = 0; i < pcds_seq->size(); ++i) {
    ...
    if (i == 0) {
      ...
      stamp_visual.second = pcl::PointCloud<pcl::PointXYZI>::Ptr(
          new pcl::PointCloud<pcl::PointXYZI>);
  ...
  for (int loop = 0; loop < refine_pose_param_.max_iter; ++loop) {
    ...
    for (size_t i = 0; i < window_size; ++i) {
      ...
      if (refine_pose_param_.downsmp_base > 0) {
        downsample_voxel(*pcds_seq->at(i).second, *pcd_surf,
                         refine_pose_param_.downsmp_base);
      }

      pose_refine::cut_voxel(surf_map, pcd_surf, pose_seq->at(i).second, i,
                             window_size, refine_pose_param_.voxel_size,
                             refine_pose_param_.eigen_thr);
    }
...
bool MultiLidarsCalib::routeIsOk(const int &cut_num, const StampedPoseVectorPtr &pose_seq,
  ...
  for (size_t i = 0; i < pose_seq->size();) {
    if (!vehicleIsStatic(pose_seq, pose_seq->at(i).second, i, th_time)) {
      ++i;
      continue;
    }

    i = i + 20;
    time_pairs_[seg_cnt].first = pose_seq->at(i).first;
    ...
    for (size_t j = i + 1; j < pose_seq->size(); ++j) {
      ...
      delta_yaw = convertRotationToEulerYPR(delta_rot).x();
  ...
  std::cerr << "Failed!! seg_cnt: " << seg_cnt << " "
            << "cut_num: " << cut_num << std::endl;
  std::cerr << "Please follow the guide for collecting data!" << std::endl;
  return false;
}
...
bool MultiLidarsCalib::vehicleIsStatic(const StampedPoseVectorPtr &pose_seq,
  ...
  for (size_t i = start; i < pose_seq->size(); i++) {
    // std::cerr << "pose_seq->at(i).first: " << pose_seq->at(i).first << std::endl;
    if (pose_seq->at(i).first - start_time >= th_time) {
      // std::cerr << "time_interval: " << start_time << ", " << pose_seq->at(i).first << std::endl;
      length = i - start;
      break;
    }
  }
  ...
  // Eigen::Vector3d ele_euler =
  // convertRotationMatrixToEulerYPR(element.block<3, 3>(0, 0)); for(size_t
  // index = start; index < start + length; ++index) {
  //   Eigen::Vector3d trans = pose_seq->at(index).second.block<3, 1>(0, 3);
  //   Eigen::Vector3d euler = convertRotationMatrixToEulerYPR(
  //     pose_seq->at(index).second.block<3, 3>(0, 0));
  //   if(((ele_trans - trans).norm() > 0.01) ||
  //       ((ele_euler - euler).norm() > 0.001)) return false;
  // }
